...
export class ProductReviewComponent implements OnInit, OnDestroy {
    ...
    this.subscriptionList$.push(
      ...
      this.store.select(getRatingCategories).subscribe(ratingCategories => {
        if (ratingCategories.product) {
          this.store.dispatch(
            this.productAction.getRatingsOptions(ratingCategories.product)
          );
        }
      })
    );
  ...
  showReviewForm() {
    if (this.isAuthenticated) {
      this.router.navigate([this.product.slug, 'write_review'], {
        queryParams: { prodId: this.product.id }
      });
    } else {
  ...
  get reviewPercent() {
    return Math.ceil(
      (+this.product.rating_summary.average_rating / this.reviewsDisplayLimit) *
        100
    );
  }
