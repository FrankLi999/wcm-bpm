...
export class PaymentService {
  ...
  makeHostedPayment(
    ...
    const params = this.buildPayubizPaymentJson(
      orderId,
      orderNumber,
      paymentId,
      orderAmount,
  ...
  makeStripePayment(
    ...
    orderId: number,
    address: Address
  ): Observable<{ order: { order_number } }> {
    const params = this.buildHostedPaymentJosn(
      orderId,
      orderNumber,
      paymentId,
      orderAmount,
  ...
  getStripeKey(paymentMethodId: number) {
    return this.http.get(
      `api/v1/hosted-payment/stripe-request?id=${paymentMethodId}`
    );
  }

  getRazorKey(paymentMethodId: number) {
    return this.http.get(
      `api/v1/hosted-payment/rzpay-request?id=${paymentMethodId}`
    );
  }

  buildHostedPaymentJosn(
    orderId: number,
    orderNumber: string,
    paymentId: string,
    orderAmount: number,
