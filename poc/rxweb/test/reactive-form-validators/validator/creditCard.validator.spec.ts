...
import { RxwebValidators, ReactiveFormConfig } from '@rxweb/reactive-form-validators';
    ...
    describe('Validator', () => {
        ...
        describe('creditCardValidator', () => {
            ...
            it("Should not error, discover card 19 digit of '6011504417586318394'",
                () => {
                    let formBuilder = new FormBuilder();
                    let formGroup = formBuilder.group({
                        'cardType': 'Discover',
                        'discoverCard': '6011504417586318394'
                    });
                    expect(RxwebValidators.creditCard({ fieldName: 'cardType' })(formGroup.controls.discoverCard)).toBeNull()
                });
            it("Should not error, discover card 19 digit of '6011345344455683539'",
                () => {
                    let formBuilder = new FormBuilder();
                    let formGroup = formBuilder.group({
                        'cardType': 'Discover',
                        'discoverCard': '6011345344455683539'
                    });
                    expect(RxwebValidators.creditCard({ fieldName: 'cardType' })(formGroup.controls.discoverCard)).toBeNull()
                });
