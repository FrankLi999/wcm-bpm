...
public class HalProcessDefinition extends HalResource<HalProcessDefinition> implements HalIdResource {
	...
	public static final HalRelation REL_SELF = HalRelation.build("self", ProcessDefinitionRestService.class,
			UriComponentsBuilder.fromPath(ProcessDefinitionRestService.PATH).path("{id}"));
	public static final HalRelation REL_DEPLOYMENT = HalRelation.build("deployment", DeploymentRestService.class,
			UriComponentsBuilder.fromPath(DeploymentRestService.PATH).path("{id}"));
	public static final HalRelation REL_DEPLOYMENT_RESOURCE = HalRelation.build("resource",
			DeploymentResourcesResource.class, UriComponentsBuilder.fromPath(DeploymentRestService.PATH).path("{deploymentId}")
					.path("resources").path("{resourceId}"));
	...
	public static HalProcessDefinition fromProcessDefinition(ProcessDefinition processDefinition,
		...
		halProcDef.contextPath = ApplicationContextPathUtil.getApplicationPathForDeployment(processEngine,
		...
		halProcDef.linker.createLink(REL_DEPLOYMENT_RESOURCE, processDefinition.getDeploymentId(),
