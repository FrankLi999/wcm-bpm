...
public class HalTaskList extends HalCollectionResource<HalTaskList> {
	...
	public static HalTaskList generate(List<Task> tasks, long count, ProcessEngine engine) {
		return fromTaskList(tasks, count).embed(HalTask.REL_ASSIGNEE, engine).embed(HalTask.REL_OWNER, engine)
				.embed(HalTask.REL_PROCESS_DEFINITION, engine).embed(HalTask.REL_CASE_DEFINITION, engine);
	}
	...
	public static HalTaskList fromTaskList(List<Task> tasks, long count) {
		...
		// links
		taskList.addLink("self", UriComponentsBuilder.fromPath(TaskRestService.PATH).build().toUriString());
