...
public class DeploymentWithDefinitionsDto extends DeploymentDto {
	...
	private static void initDeployedResourceLists(DeploymentWithDefinitions deployment,
			DeploymentWithDefinitionsDto dto) {
		...
		if (deployedProcessDefinitions != null) {
			...
			for (ProcessDefinition processDefinition : deployedProcessDefinitions) {
				dto.deployedProcessDefinitions.put(processDefinition.getId(),
						ProcessDefinitionDto.fromProcessDefinition(processDefinition));
			}
		}
		...
		if (deployedCaseDefinitions != null) {
			...
			for (CaseDefinition caseDefinition : deployedCaseDefinitions) {
				dto.deployedCaseDefinitions.put(caseDefinition.getId(),
						CaseDefinitionDto.fromCaseDefinition(caseDefinition));
			}
		}
		...
		if (deployedDecisionDefinitions != null) {
			...
			for (DecisionDefinition decisionDefinition : deployedDecisionDefinitions) {
				dto.deployedDecisionDefinitions.put(decisionDefinition.getId(),
						DecisionDefinitionDto.fromDecisionDefinition(decisionDefinition));
			}
		}

		List<DecisionRequirementsDefinition> deployedDecisionRequirementsDefinitions = deployment
		...
		if (deployedDecisionRequirementsDefinitions != null) {
			...
			for (DecisionRequirementsDefinition drd : deployedDecisionRequirementsDefinitions) {
				dto.deployedDecisionRequirementsDefinitions.put(drd.getId(),
						DecisionRequirementsDefinitionDto.fromDecisionRequirementsDefinition(drd));
			}
		}
	}
}
