...
public class TaskQueryDto extends AbstractQueryDto<TaskQuery> {
	...
	public TaskQueryDto(ObjectMapper objectMapper, Map<String, String[]> queryParameters) {
			...
			for (VariableQueryParameterDto variableQueryParam : taskVariables) {
				...
				if (op.equals(VariableQueryParameterDto.EQUALS_OPERATOR_NAME)) {
					...
					throw new InvalidRequestException(HttpStatus.BAD_REQUEST,
							"Invalid task variable comparator specified: " + op);
				}
		...
		if (processVariables != null) {
			for (VariableQueryParameterDto variableQueryParam : processVariables) {
				...
				if (op.equals(VariableQueryParameterDto.EQUALS_OPERATOR_NAME)) {
					...
					throw new InvalidRequestException(HttpStatus.BAD_REQUEST,
							"Invalid process variable comparator specified: " + op);
				}
		...
		if (caseInstanceVariables != null) {
			for (VariableQueryParameterDto variableQueryParam : caseInstanceVariables) {
				...
				if (op.equals(VariableQueryParameterDto.EQUALS_OPERATOR_NAME)) {
					...
					throw new InvalidRequestException(HttpStatus.BAD_REQUEST,
							"Invalid case variable comparator specified: " + op);
				}
			}
		}
	}
	...
	public static List<SortingDto> convertQueryOrderingPropertiesToSortingDtos(
	...
	public static String sortByValueForQueryEntityRelationCondition(QueryEntityRelationCondition relationCondition) {
		...
		if (VariableInstanceQueryProperty.EXECUTION_ID.equals(property)
				&& TaskQueryProperty.PROCESS_INSTANCE_ID.equals(comparisonProperty)) {
			return SORT_BY_PROCESS_VARIABLE;
		} else if (VariableInstanceQueryProperty.EXECUTION_ID.equals(property)
				&& TaskQueryProperty.EXECUTION_ID.equals(comparisonProperty)) {
			return SORT_BY_EXECUTION_VARIABLE;
		} else if (VariableInstanceQueryProperty.TASK_ID.equals(property)
				&& TaskQueryProperty.TASK_ID.equals(comparisonProperty)) {
			return SORT_BY_TASK_VARIABLE;
		} else if (VariableInstanceQueryProperty.CASE_EXECUTION_ID.equals(property)
				&& TaskQueryProperty.CASE_INSTANCE_ID.equals(comparisonProperty)) {
			return SORT_BY_CASE_INSTANCE_VARIABLE;
		} else if (VariableInstanceQueryProperty.CASE_EXECUTION_ID.equals(property)
				&& TaskQueryProperty.CASE_EXECUTION_ID.equals(comparisonProperty)) {
			return SORT_BY_CASE_EXECUTION_VARIABLE;
		} else {
			throw new RestException("Unknown relation condition for task query  with query property " + property
					+ " and comparison property " + comparisonProperty);
		}
	}

	public static Map<String, Object> sortParametersForVariableOrderProperty(
