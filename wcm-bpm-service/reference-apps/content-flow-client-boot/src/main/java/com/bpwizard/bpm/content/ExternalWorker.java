...
public class ExternalWorker {
	...
	public void fetchExternalTask() throws IOException, InterruptedException {
		...
		topic.put("topicName", "creditScoreChecker");
		...
		for (JsonNode lockedTask : answer) {
			...
			System.out.println(String.format(">>>>>>>>>>>>> work on task %s for %s milliseconds", taskId, duration));
			
			Thread.sleep(duration);

			LOGGER.info("task {} finished calculation", taskId);
			System.out.println(String.format(">>>>>>>>>>> task %s finished calculation", taskId));
			if (duration % 5 == 0) {
				...
				System.out.println(String.format("duration is %s, create incident for task %s", duration, taskId));
				...
				System.out.println(String.format("complete task %s", taskId));
				int defaultScore = 5;// externalTask.getVariable("defaultScore");
				List<Integer> creditScores = new ArrayList<>(Arrays.asList(defaultScore, 9, 1, 4, 10));
				
				ArrayNode creditScoresNode = mapper.createArrayNode();
				// creditScores.forEach(score -> creditScoresNode.add(score));
				creditScores.forEach(score -> creditScoresNode.add(score));
				ObjectNode variables = mapper.createObjectNode();
				
				variables.set("creditScores", creditScoresNode);
