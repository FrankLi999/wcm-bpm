...
public class CustomerCreditWorker {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		while (running) {
			List<LockedExternalTaskDto> tasks = client.target(BASE_URL + "external-task/fetchAndLock")
					.request(MediaType.APPLICATION_JSON) //
					.post(Entity.entity(fetchExternalTasksDto, MediaType.APPLICATION_JSON), //
			...
			for (LockedExternalTaskDto task : tasks) {
				if ("customer-credit".equals(task.getTopicName())) {
					...
					client.target("http://localhost:8092/rest/engine/default/external-task").path(task.getId()) //
							.path("complete").request(MediaType.APPLICATION_JSON) //
							.post(Entity.entity(completeTaskDto, MediaType.APPLICATION_JSON));
