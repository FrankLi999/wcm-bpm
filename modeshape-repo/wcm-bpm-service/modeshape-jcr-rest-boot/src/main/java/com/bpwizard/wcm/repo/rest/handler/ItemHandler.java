...
public abstract class ItemHandler extends AbstractHandler {
    ...
    protected Node addNode( Node parentNode,
                            String nodeName,
                            JsonNode jsonNode ) throws RepositoryException, IOException {
        Node newNode;
        ...
        for (Iterator<?> iter = properties.fieldNames(); iter.hasNext();) {
    ...
    protected List<JsonChild> getChildren( JsonNode jsonNode ) throws IOException {
        List<JsonChild> children;
        if (jsonNode.isArray()) {
        	ArrayNode childrenArray = (ArrayNode)jsonNode.get(CHILD_NODE_HOLDER);
            ...
            for (int i = 0; i < childrenArray.size(); i++) {
                ...
                String childName = child.fieldNames().next().toString();
        	...
        	JsonNode childrenObject = jsonNode.get(CHILD_NODE_HOLDER);
            children = new ArrayList<>(childrenObject.size());
            for (Iterator<?> iterator = childrenObject.fieldNames(); iterator.hasNext();) {
    ...
    protected JsonNode getProperties( JsonNode jsonNode ) throws IOException {
        return jsonNode.has(PROPERTIES_HOLDER) ? jsonNode.get(PROPERTIES_HOLDER) : this.mapper.createObjectNode();
    }
    ...
    protected void setPropertyOnNode( Node node,
                                      String propName,
                                      JsonNode value ) throws RepositoryException, IOException {
    ...
    private Set<String> updateMixins( Node node,
                                      JsonNode mixinsJsonValue ) throws IOException, RepositoryException {
    ...
    private Object convertToJcrValues( Node node,
                                        JsonNode value,
                                        boolean encoded ) throws RepositoryException, IOException {
        if (value.isNull() || (value instanceof ArrayNode && ((ArrayNode)value).size() == 0)) {
    ...
    protected Item updateItem( Item item,
                               JsonNode jsonItem ) throws RepositoryException, IOException {
    ...
    protected Node updateNode( Node node,
                               JsonNode jsonNode,
                               VersionableChanges changes ) throws RepositoryException, IOException {
    ...
    private void updateChildren( Node node,
                                 JsonNode jsonNode,
                                 VersionableChanges changes ) throws IOException, RepositoryException {
