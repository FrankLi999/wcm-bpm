...
export class ResourceTypeLayoutComponent implements OnInit {
  ...
  groupType(group: BaseFormGroup): string {
    ...
    if ((group as StepsFormGroup).steps!== undefined) {
      return "steps";
    }
    if ((group as TabsFormGroup).tabs !== undefined) {
      return "tabs";
    }
    if ((group as RowsFormGroup).rows !== undefined) {
      return "rows";
    }
  ...
  drop(event: CdkDragDrop<ControlField[]|TemplateField[]>) {
    if (event.previousContainer === event.container) {
      moveItemInArray((event.container.data as TemplateField[]), event.previousIndex, event.currentIndex);
    } else if ("palletFields" !== event.previousContainer.id) {
      transferArrayItem((event.previousContainer.data as TemplateField[]),
        (event.container.data as TemplateField[]),
        event.previousIndex,
        event.currentIndex);
    } else {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result) {
          ...
          console.log(result);
          copyArrayItem([result.templateField],
            (event.container.data as TemplateField[]),
            0,
            event.currentIndex);
            // event.container.data[event.currentIndex] = result.templateField;
        }
      });
    }
  }
  ...
  deleteTargetField(index:number, fields: TemplateField[]) {
      fields.splice(index, 1);
  }

  editTargetField(index:number, fields: TemplateField[]) {
    const dialogRef = this.dialog.open(ResourceFieldDialog, {
      width: '500px',
      data: {
        controlField: this.getControlField(fields[index].controlName),
        templateField: fields[index]}
    });

    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        fields[index] = result.templateField;
      }
    });
  }
  ...
  getResourceFieldHint(templateField: TemplateField): string {
      let hint = '';
  ...
  getResourceFieldFlags(templateField: TemplateField): string {
      let fieldFlag = '';
  ...
  addNewRow(numOfColumn: number) {
    ...
    this.resourceType.formGroups[this.resourceType.formGroups.length - 1] as RowsFormGroup:
        { rows: []} as RowsFormGroup;
  ...
  addNewTab(numOfColumn: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
        if (result) {
            ...
            this.resourceType.formGroups[this.resourceType.formGroups.length - 1] as TabsFormGroup:
                { tabs: []} as TabsFormGroup;
  ...
  addNewStep(numOfColumn: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
        if (result) {
          ...
          this.resourceType.formGroups[this.resourceType.formGroups.length - 1] as StepsFormGroup:
              { steps: []} as StepsFormGroup;
        
          (<StepsFormGroup>steps).steps.push({
            formGroups: [{
                columns: []
              } as FormRow],
              stepName: result
          });
          let step = (<StepsFormGroup>steps).steps[(<StepsFormGroup>steps).steps.length - 1];
          for (let i = 0; i < numOfColumn; i++) {
              let fieldGroupId = "fieldgroup" + this.nextFieldGroupId++; 
