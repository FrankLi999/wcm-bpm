...
public class HalLinker {
	 ...
	 *
	 * @param rel        the {@link HalRelation} for which a link should be
	 *                   constructed
	...
	/**
	 * Resolves a relation. Locates a HalLinkResolver for resolving the set of all
	 * linked resources in the relation.
	 *
	 * @param relation      the relation to resolve
	 * @param processEngine the process engine to use
	 * @return the list of resolved resources
	 * @throws RuntimeException if no HalLinkResolver can be found for the linked
	 *                          resource type.
	 */
	public List<HalResource<?>> resolve(HalRelation relation, ProcessEngine processEngine) {
		...
		if (linkResolver != null) {
			...
			throw new RuntimeException(
					"Cannot find HAL link resolver for resource type '" + relation.resourceType + "'.");
		}
	}

	/**
	 * merge the links of an embedded resource into this linker. This is useful when
	 * building resources which are actually resource collections. You can then
	 * merge the relations of all resources in the collection and the unique the set
	 * of linked resources to embed.
	 *
	 * @param embedded the embedded resource for which the links should be merged
	 *                 into this linker.
	 */
