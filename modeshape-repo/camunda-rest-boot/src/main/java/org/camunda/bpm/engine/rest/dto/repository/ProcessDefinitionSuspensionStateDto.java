...
public class ProcessDefinitionSuspensionStateDto extends SuspensionStateDto {
	...
	public void updateSuspensionState(ProcessEngine engine) {
		...
		if (processDefinitionId != null) {
			...
			if (getSuspended()) {
				repositoryService.suspendProcessDefinitionById(processDefinitionId, includeProcessInstances,
						delayedExecutionDate);
			} else {
				repositoryService.activateProcessDefinitionById(processDefinitionId, includeProcessInstances,
						delayedExecutionDate);
			}
		} else
		...
		if (processDefinitionKey != null) {
			...
			if (getSuspended()) {
				repositoryService.suspendProcessDefinitionByKey(processDefinitionKey, includeProcessInstances,
						delayedExecutionDate);
			} else {
				repositoryService.activateProcessDefinitionByKey(processDefinitionKey, includeProcessInstances,
						delayedExecutionDate);
			}
		} else {
