...
public class HalTask extends HalResource<HalTask> {
	...
	public static HalRelation REL_SELF = HalRelation.build("self", TaskRestService.class,
			UriComponentsBuilder.fromPath(TaskRestService.PATH).path("{id}"));
	public static HalRelation REL_ASSIGNEE = HalRelation.build("assignee", UserRestService.class,
			UriComponentsBuilder.fromPath(UserRestService.PATH).path("{id}"));
	...
	public static HalRelation REL_EXECUTION = HalRelation.build("execution", ExecutionRestService.class,
			UriComponentsBuilder.fromPath(ExecutionRestService.PATH).path("{id}"));
	public static HalRelation REL_PARENT_TASK = HalRelation.build("parentTask", TaskRestService.class,
			UriComponentsBuilder.fromPath(TaskRestService.PATH).path("{id}"));
	public static HalRelation REL_PROCESS_DEFINITION = HalRelation.build("processDefinition",
			ProcessDefinitionRestService.class, UriComponentsBuilder.fromPath(ProcessDefinitionRestService.PATH).path("{id}"));
	public static HalRelation REL_PROCESS_INSTANCE = HalRelation.build("processInstance",
			ProcessInstanceRestService.class, UriComponentsBuilder.fromPath(ProcessInstanceRestService.PATH).path("{id}"));
	public static HalRelation REL_CASE_INSTANCE = HalRelation.build("caseInstance", CaseInstanceRestService.class,
			UriComponentsBuilder.fromPath(CaseInstanceRestService.PATH).path("{id}"));
	public static HalRelation REL_CASE_EXECUTION = HalRelation.build("caseExecution", CaseExecutionRestService.class,
			UriComponentsBuilder.fromPath(CaseExecutionRestService.PATH).path("{id}"));
	public static HalRelation REL_CASE_DEFINITION = HalRelation.build("caseDefinition", CaseDefinitionRestService.class,
			UriComponentsBuilder.fromPath(CaseDefinitionRestService.PATH).path("{id}"));
	public static HalRelation REL_IDENTITY_LINKS = HalRelation.build("identityLink", IdentityRestService.class,
			UriComponentsBuilder.fromPath(TaskRestService.PATH).path("{taskId}").path("identity-links"));
	...
	public static HalTask generate(Task task, ProcessEngine engine) {
		return fromTask(task).embed(HalTask.REL_PROCESS_DEFINITION, engine).embed(HalTask.REL_CASE_DEFINITION, engine)
				.embed(HalTask.REL_IDENTITY_LINKS, engine).embed(HalIdentityLink.REL_USER, engine)
