...
public class Form extends ResourceMixin implements HasName {
	...
	public BaseFormGroup[] getFormLayout() {
		return formLayout;
	}

	public void setFormLayout(BaseFormGroup[] formLayout) {
	...
	public JsonNode toJson() {
		...
		jsonNode.put(JcrConstants.JCR_PRIMARY_TYPE, "bpw:formType");

		super.toJson(jsonNode);
		if (this.getFormLayout() != null && this.getFormLayout().length > 0) {
			ObjectNode elementGroupNode = JsonUtils.createObjectNode();
			ObjectNode elementGroupNodeChildren = JsonUtils.createObjectNode();
			children.set("formLayout", elementGroupNode);
			...
			this.addFormGroups(elementGroupNodeChildren, this.getFormLayout());

		}

		if (this.getFormControls() != null && this.getFormControls().size() > 0) {
			...
			children.set("formControls", elementFolderNode);
			elementFolderNode.put(JcrConstants.JCR_PRIMARY_TYPE, "bpw:elementFolder");
			for (String controlName : this.getFormControls().keySet()) {
				this.addControl(elementFolderNodeChildren, this.getFormControls().get(controlName));
			}
		}
	...
	private void addColumn(ObjectNode rowNode, FormColumn column, String columnNameName) {
		...
		if (column.getFormGroups() != null && column.getFormGroups().length > 0) {
			...
			this.addFormGroups(formGroupsChildren, this.getFormLayout());
		}
