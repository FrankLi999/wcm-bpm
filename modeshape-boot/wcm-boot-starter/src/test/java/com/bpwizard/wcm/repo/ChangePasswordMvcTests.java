...
public class ChangePasswordMvcTests extends AbstractMvcTests {
	...
	public void testChangePassword() throws Exception {
		...
		mvc.perform(post("/api/core/users/{id}/password", UNVERIFIED_USER_ID)
				...
				.content(SecurityUtils.toJson(changePasswordForm(USER_PASSWORD))))
	...
	public void testAdminChangePasswordAnotherUser() throws Exception {
		...
		mvc.perform(post("/api/core/users/{id}/password", UNVERIFIED_USER_ID)
				...
				.contentType(MediaType.APPLICATION_JSON)
				.content(SecurityUtils.toJson(changePasswordForm(ADMIN_PASSWORD))))
				.andExpect(status().is(204))
				.andExpect(header().string(SecurityUtils.TOKEN_RESPONSE_HEADER_NAME, containsString(".")));
	...
	public void testChangePasswordInvalidData() throws Exception {
		...
		mvc.perform(post("/api/core/users/{id}/password", UNVERIFIED_USER_ID)
				.header(HttpHeaders.AUTHORIZATION, tokens.get(UNVERIFIED_USER_ID))
				.contentType(MediaType.APPLICATION_JSON)
				.content(SecurityUtils.toJson(new ChangePasswordForm())))
