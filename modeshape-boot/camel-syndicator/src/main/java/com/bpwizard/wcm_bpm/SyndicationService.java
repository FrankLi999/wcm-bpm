...
public class SyndicationService {
    ...
    public void syndicate(Syndication syndication) throws Exception {
    	...
    	Timestamp endTimestamp = new Timestamp(System.currentTimeMillis());
    	 DataSource authoringDs = DataSourceBuilder.create().url(syndication.getAuthoringDBUrl())
    	 	.driverClassName("com.mysql.jdbc.Driver")
    	 	.username(syndication.getAuthoringDBUser())
    	 	.password(syndication.getAuthoringDBPassword()).build();
    	 
    	 JdbcTemplate authoringJdbcTemplate = new JdbcTemplate(authoringDs);
    	 
    	 DataSource renderingDs = DataSourceBuilder.create().url(syndication.getRenderingDBUrl())
    	    	 	.driverClassName("com.mysql.jdbc.Driver")
    	    	 	.username(syndication.getRenderingDBUser())
    	    	 	.password(syndication.getRenderingDBPassword()).build();
    	 JdbcTemplate renderingJdbcTemplate = new JdbcTemplate(renderingDs);
    	 
    	 
    	 List<WcmEvent> cndEvents = getCndAfter(authoringJdbcTemplate, syndication.getLastSyndication(), endTimestamp);
    	...
    	int pageSize = 20;
 		...
 		do {
 			...
 			wcmEvents = getWcmEventAfter(authoringJdbcTemplate, syndication, endTimestamp, pageIndex, pageSize);
 			actualBatchSize = wcmEvents.size();
 			if (actualBatchSize > 0) {
 				for (WcmEvent wcmEvent: wcmEvents) {
 					
 					publish(syndication,
 							authoringJdbcTemplate, 
 							renderingJdbcTemplate, 
 							getJcrNode(authoringJdbcTemplate, 
 							wcmEvent.getId()), 
 							wcmEvent,
 							token);
 				}
