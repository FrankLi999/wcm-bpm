  ...
  const formLayout = mapLayout(
    ...
    (layoutItem, index, layoutPointer) => {
          ...
          if (hasOwn(newNode.options, "errorMessages")) {
            ...
            if (typeof newNode.options.validationMessage === "string") {
              newNode.options.validationMessages =
                newNode.options.validationMessage;
            } else {
              newNode.options.validationMessages = {};
              Object.keys(newNode.options.validationMessage).forEach((key) => {
                ...
                const newKey =
                  code === "0"
                    ...
                    ? "format"
                    : code + "";
                newNode.options.validationMessages[newKey] =
                  newNode.options.validationMessage[key];
              });
            }
        ...
        if (hasOwn(newNode, "key")) {
          newNode.dataPointer =
            newNode.key === "*"
              ? newNode.key
              : JsonPointer.compile(
                  JsonPointer.parseObjectPath(newNode.key),
                  "-"
                );
          delete newNode.key;
          ...
          if (childDataPointer) {
            newNode.dataPointer = childDataPointer.slice(
              0,
              childDataPointer.lastIndexOf("/-")
            );
          }
        }
      }
      ...
      if (hasOwn(newNode, "dataPointer")) {
        ...
        const nodeValue = JsonPointer.get(
          jsf.formValues,
        ...
        newNode.dataPointer = JsonPointer.toGenericPointer(
          newNode.dataPointer,
          jsf.arrayMap
        );
        ...
        const recursive =
          !shortDataPointer.length || shortDataPointer !== newNode.dataPointer;
        ...
        if (nodeDataMap.has("schemaPointer")) {
          ...
          schemaPointer = JsonPointer.toSchemaPointer(
            shortDataPointer,
            jsf.schema
          );
        ...
        if (nodeSchema) {
          if (!hasOwn(newNode, "type")) {
            ...
            newNode.type = getInputType(nodeSchema, newNode);
            console.error(
              `error: widget type "${oldWidgetType}" ` +
          ...
          if (newNode.type === "checkboxes" && hasOwn(nodeSchema, "items")) {
            ...
            newNode.options.maxItems = Math.min(
              nodeSchema.maxItems || 1000,
              newNode.options.maxItems || 1000
            );
            newNode.options.minItems = Math.max(
              nodeSchema.minItems || 0,
              newNode.options.minItems || 0
            );
            newNode.options.listItems = Math.max(
              newNode.options.listItems || 0,
              isArray(nodeValue) ? nodeValue.length : 0
            );
            newNode.options.tupleItems = isArray(nodeSchema.items)
              ? nodeSchema.items.length
              : 0;
        ...
        if (
          ...
          if (newNode.items.length > 1) {
            ...
            for (let i = newNode.items.length - 1; i >= 0; i--) {
              ...
              if (
                hasOwn(subItem, "dataPointer") &&
                subItem.dataPointer.slice(0, itemRefPointer.length) ===
                  itemRefPointer
              ) {
                const arrayItem = newNode.items.splice(i, 1)[0];
                arrayItem.dataPointer =
                  newNode.dataPointer +
                  "/-" +
            ...
            if (arrayItemGroup.length) {
              newNode.items.push({
                ...
                arrayItem: true,
                arrayItemType:
                  newNode.options.tupleItems > newNode.items.length
                    ? "tuple"
                    : "list",
            ...
            if (!newNode.items[0].dataPointer) {
              newNode.items[0].dataPointer = JsonPointer.toGenericPointer(
                itemRefPointer,
                jsf.arrayMap
              );
            }
            ...
            newNode.items[0].arrayItemType = newNode.options.tupleItems
              ? "tuple"
              : "list";
          }
          ...
          if (!hasOwn(jsf.layoutRefLibrary, itemRefPointer)) {
            jsf.layoutRefLibrary[itemRefPointer] = cloneDeep(
              ...
              (item, key) => {
                ...
                if (recursive) {
                  if (hasOwn(item, "dataPointer")) {
                    item.dataPointer = item.dataPointer.slice(
                      itemRefPointer.length
                    );
                  }
                }
              },
          ...
          if (
            ...
            if (newNode.options.title) {
              ...
              const parentSchema = getFromSchema(
                jsf.schema,
                newNode.dataPointer,
                "parentSchema"
              );
              if (hasOwn(parentSchema, "title")) {
                ...
                buttonText +=
                  " to " + fixTitle(pointerArray[pointerArray.length - 2]);
              }
            }
            newNode.items.push({
              _id: uniqueId(),
        ...
        const parentType: string = JsonPointer.get(
          jsf.layout,
          layoutPointer,
          0,
          -2
        ...
        if (!hasOwn(newNode, "type")) {
          newNode.type = inArray(parentType, ["tabs", "tabarray"])
            ? "tab"
            : "array";
        }
      ...
      // Todo, validate it
      if (newNode.options.functions) {
        if (newNode.options.functions.onClick) {
          newNode.options.onClick = new Function(
            newNode.options.functions.onClick.arguments,
            newNode.options.functions.onClick.body
          );
        }

        if (newNode.options.functions.onBlur) {
          newNode.options.onBlur = new Function(
            newNode.options.functions.onBlur.arguments,
            newNode.options.functions.onBlur.body
          );
        }
        if (newNode.options.functions.validateFn) {
          newNode.options.validateFn = new Function(
            newNode.options.functions.validateFn.arguments,
            newNode.options.functions.validateFn.body
          );
        }
      }
      return newNode;
...
export function buildLayoutFromSchema(
  ...
  if (newNode.dataType === "object") {
      ...
      propertyKeys
        ...
        .forEach((key) => {
          ...
          const innerItem = buildLayoutFromSchema(
            jsf,
            widgetLibrary,
            isObject(nodeValue) ? nodeValue[key] : null,
    ...
    if (
      !newNode.options.minItems &&
      isInputRequired(jsf.schema, schemaPointer)
    ) {
    ...
    newNode.options.tupleItems = isArray(schema.items)
      ? schema.items.length
      : 0;
    if (newNode.options.maxItems <= newNode.options.tupleItems) {
      newNode.options.tupleItems = newNode.options.maxItems;
        ...
        if (removable && i >= newNode.options.minItems) {
          if (!hasOwn(jsf.layoutRefLibrary, itemRefPointer)) {
            ...
            jsf.layoutRefLibrary[itemRefPointer] = buildLayoutFromSchema(
              jsf,
              widgetLibrary,
              isArray(nodeValue) ? nodeValue[i] : null,
    ...
    if (additionalItemsSchemaPointer) {
      ...
      const itemRecursive =
        !itemRefPointer.length || itemRefPointer !== shortDataPointer + "/-";
      ...
      if (
        itemRefPointer.length &&
        !hasOwn(jsf.layoutRefLibrary, itemRefPointer)
      ) {
        // Set to null first to prevent recursive reference from causing endless loop
        jsf.layoutRefLibrary[itemRefPointer] = null;
        jsf.layoutRefLibrary[itemRefPointer] = buildLayoutFromSchema(
      ...
      if (
        ...
        let buttonText = (
          (jsf.layoutRefLibrary[itemRefPointer] || {}).options || {}
        ).title;
    ...
    if (newNode.options.add) {
      ...
      if (hasOwn(parentSchema, "title")) {
        ...
        const pointerArray = JsonPointer.parse(newNode.dataPointer);
        buttonText =
          "Add to " + fixTitle(pointerArray[pointerArray.length - 2]);
      }
    }
    ...
    if (isNumber(JsonPointer.get(jsf.schema, schemaPointer, 0, -1).maxItems)) {
      newNode.options.maxItems = JsonPointer.get(
        jsf.schema,
        schemaPointer,
        0,
        -1
...
export function mapLayout(layout, fn, layoutPointer = "", rootLayout = layout) {
  ...
  forEach(layout, (item, index) => {
    ...
    if (itemsArray.length) {
      newNode.items = mapLayout(
        itemsArray,
        fn,
        newLayoutPointer + "/items",
        rootLayout
      );
    }
  ...
  if (titleMap) {
    if (isArray(titleMap)) {
      if (enumList) {
        ...
        if (!fieldRequired) {
          hasEmptyValue = !!newTitleMap.filter(
    ...
    if (flatList) {
      ...
      newTitleMap = newTitleMap.reduce((groupTitleMap, title) => {
        if (hasOwn(title, "group")) {
          if (
            title.group !==
            (groupTitleMap[groupTitleMap.length - 1] || {}).group
          ) {
            groupTitleMap.push({
              group: title.group,
              items: title.items || [],
          ...
          if (hasOwn(title, "name") && hasOwn(title, "value")) {
            groupTitleMap[groupTitleMap.length - 1].items.push({
              name: title.name,
              value: title.value,
            });
            if (title.value === undefined || title.value === null) {
