...
export class JsonSchemaFormService {
  ...
  initializeControl(ctx: any, bind = true): boolean {
    ...
    if (ctx.formControl) {
      if (ctx.options.validateFn) {
        ctx.formControl.setValidators(
          ctx.formControl.validator
            ? [ctx.formControl.validator, ctx.options.validateFn as ValidatorFn]
            : (ctx.options.validateFn as ValidatorFn)
        );
      }
      if (ctx.options.aysncValidateFn) {
        ctx.formControl.AsyncValidators(
          ctx.formControl.asyncValidator
            ? [
                ctx.formControl.asyncValidator,
                ctx.options.aysncValidateFn as AsyncValidatorFn,
              ]
            : (ctx.options.aysncValidateFn as AsyncValidatorFn)
        );
      }
      if (
        ctx.options.customValidationMessages &&
        ctx.options.customValidationMessages[this.language]
      ) {
        Object.keys(
          ctx.options.customValidationMessages[this.language]
        ).forEach(
          (key) =>
            (ctx.options.validationMessages[key] =
              ctx.options.customValidationMessages[this.language][key])
        );
      }
      ...
      ctx.formControl.valueChanges.subscribe((value) => {
        if (!!value) {
          ...
          this.crossFieldUpdate(ctx, value);
        }
      });
    } else {
  ...
  crossFieldUpdate(ctx: any, value: any) {
