...
export class JsonSchemaFormComponent
  ...
  @Input() readonly: boolean; // Th who form is readonly
   ...
   */
  initializeForm() {
    if (this.data || this.form || this.jsf.data) {
   ...
   * 1. form.schema - Single input / JSON Form style
   */
  ...
  private activateForm() {
    ...
    if (this.jsf.formGroup) {
      ...
      this.dataSubscription = this.jsf.dataChanges.subscribe((data) => {
      ...
      this.jsf.isValidChanges.subscribe((isValid) =>
