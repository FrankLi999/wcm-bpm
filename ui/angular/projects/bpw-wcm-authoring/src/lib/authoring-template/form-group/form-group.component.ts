...
import { WcmTabEditorDialog } from "./tab-editor-dialog.component";
import { WcmStepEditorDialog } from "./step-editor-dialog.component";

@Component({
  selector: "wcm-authoring-form-group",
  templateUrl: "./form-group.component.html",
  styleUrls: ["./form-group.component.scss"],
})
export class WcmFormGroupComponent implements OnInit {
  @Input() elements: { [key: string]: FormControl };
  @Input() properties: { [key: string]: FormControl };
  @Input() elementGroups: BaseFormGroup[];
  ...
  editStep(step: FormStep) {
    const dialogRef = this.dialog.open(WcmTabEditorDialog, {
      width: "500px",
      data: step.stepName,
    });
  ...
  addNewRow(numOfColumn: number) {
    let rows = this.isRows(this.elementGroups[this.elementGroups.length - 1])
      ? (this.elementGroups[this.elementGroups.length - 1] as FormRows)
      : ({ rows: [] } as FormRows);
    rows.rows.push({
      columns: [],
    });
    ...
    if (!this.isRows(this.elementGroups[this.elementGroups.length - 1])) {
      this.elementGroups.push(rows);
    }
    return false;
  }
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        let tabs = this.isTabs(
          this.elementGroups[this.elementGroups.length - 1]
        )
          ? (this.elementGroups[this.elementGroups.length - 1] as FormTabs)
          : ({ tabs: [] } as FormTabs);
        tabs.tabs.push({
          formGroups: [
            {
        ...
        if (!this.isTabs(this.elementGroups[this.elementGroups.length - 1])) {
          this.elementGroups.push(tabs);
        }
      }
    });
    return false;
  ...
  addNewStep(numOfColumn: number) {
    const dialogRef = this.dialog.open(WcmStepEditorDialog, {
      width: "500px",
      data: "step_name",
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        let steps = this.isStepers(
          ...
          ? (this.elementGroups[this.elementGroups.length - 1] as FormSteps)
          : ({ steps: [] } as FormSteps);
        ...
        if (
          !this.isStepers(this.elementGroups[this.elementGroups.length - 1])
        ) {
          this.elementGroups.push(steps);
        }
      }
    });
    return false;
