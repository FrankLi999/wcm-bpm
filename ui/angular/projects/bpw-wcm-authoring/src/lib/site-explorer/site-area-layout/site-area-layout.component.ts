...
export class SiteAreaLayoutComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.store
      ...
      .subscribe((rts: { [key: string]: RenderTemplate }) => {
        if (rts) {
          for (let prop in rts) {
  ...
  //   let contentAreaLayout = this.siteArea.elements["contentAreaLayout"]
  //     ? this.contentAreaLayouts[this.siteArea.elements["contentAreaLayout"]]
  //     : null;
  //   return {
