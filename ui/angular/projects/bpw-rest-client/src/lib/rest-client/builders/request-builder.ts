    ...
    return function(target: RestClient, propertyKey: string, descriptor: any) {
      ...
      descriptor.value = function(...args: any[]) {
        ...
        if (this.getApiBaseUrl() !== null) {
          ...
          if (
            apiBaseUrl.indexOf("/") === apiBaseUrl.length - 1 &&
            resUrl.indexOf("/") === 0
          ) {
        ...
        let headers: AngularHeaders = new AngularHeaders(
        ...
        let observable: Observable<HttpEvent<any>> = (<HttpClient>(
          this.httpClient
        )).request(req);
        ...
        if (descriptor.timeout) {
          descriptor.timeout.forEach((descriptor_timeout: number) => {
            observable = observable.pipe(timeout(descriptor_timeout));
          });
        }
