...
import { updateUserShortcuts } from 'bpw-auth/store/userSlice';
import { selectNavigation } from '../../store/navigationSlice';
import BpwAnimateGroup from '../BpwAnimateGroup';
...
function BpwShortcuts(props) {
  ...
  const shortcutItems = shortcuts ? shortcuts.map((id) => BpwUtils.findById(navigationData, id)) : [];

  useEffect(() => {
    function flattenNavigation() {
      setNavigation(BpwUtils.getFlatNavigation(navigationData));
    }
    ...
    <div className={clsx(classes.root, props.variant, 'flex flex-1', props.variant === 'vertical' && 'flex-grow-0 flex-shrink', props.className)}>
      ...
      >
        ...
        <Tooltip title="Click to add/remove shortcut" placement={props.variant === 'horizontal' ? 'bottom' : 'left'}>
          <IconButton className="w-40 h-40 p-0" aria-owns={addMenu ? 'add-menu' : null} aria-haspopup="true" onClick={addMenuClick}>
      ...
      >
        ...
        {searchText.length === 0 &&
          shortcutItems.map((item) => item && <ShortcutMenuItem key={item.id} item={item} onToggle={() => toggleInShortcuts(item.id)} />)}
      </Menu>
    </div>
  );
}

BpwShortcuts.propTypes = {};
BpwShortcuts.defaultProps = {
  variant: 'horizontal',
};

export default React.memo(BpwShortcuts);
