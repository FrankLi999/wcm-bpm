...
function BpwNavHorizontalCollapse(props) {
                      ...
                      {item.children && (
                        <ul className={clsx(classes.children, 'popper-navigation-list', dense && 'dense', 'px-0')}>
                          {item.children.map((_item) => (
                            <BpwNavItem key={_item.id} type={`horizontal-${_item.type}`} item={_item} nestedLevel={nestedLevel + 1} dense={dense} />
                          ))}
                        </ul>
                      )}
                    </Paper>
...
BpwNavHorizontalCollapse.propTypes = {
...
BpwNavHorizontalCollapse.defaultProps = {};

const NavHorizontalCollapse = withRouter(React.memo(BpwNavHorizontalCollapse));
