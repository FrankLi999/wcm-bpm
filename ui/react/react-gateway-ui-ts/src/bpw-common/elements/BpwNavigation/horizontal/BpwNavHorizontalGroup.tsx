...
function BpwNavHorizontalGroup(props) {
  ...
  function isUrlInChildren(parent, url) {
    if (!parent.children || url === undefined) {
      return false;
    }
            ...
            >
              ...
              {nestedLevel > 0 && (
                <IconButton disableRipple className="w-16 h-16 ltr:ml-4 rtl:mr-4 p-0" color="inherit">
                  <Icon className="text-14 arrow-icon">{theme.direction === 'ltr' ? 'keyboard_arrow_right' : 'keyboard_arrow_left'}</Icon>
                </IconButton>
              )}
            </ListItem>
          </div>
                    ...
                    {item.children && (
                      <ul className={clsx(classes.children, 'popper-navigation-list', dense && 'dense', 'px-0')}>
                        {item.children.map((_item) => (
                          <BpwNavItem key={_item.id} type={`horizontal-${_item.type}`} item={_item} nestedLevel={nestedLevel} dense={dense} />
                        ))}
                      </ul>
                    )}
                  </Paper>
...
BpwNavHorizontalGroup.propTypes = {
...
BpwNavHorizontalGroup.defaultProps = {};
export default withRouter(React.memo(BpwNavHorizontalGroup));
// const NavHorizontalGroup = withRouter(React.memo(BpwNavHorizontalGroup));
