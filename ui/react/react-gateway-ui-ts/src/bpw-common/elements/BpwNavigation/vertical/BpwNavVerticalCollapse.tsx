...
function BpwNavVerticalCollapse(props) {
  ...
  return (
    <ul className={clsx(classes.root, open && 'open')}>
      ...
      >
        ...
        <ListItemText className="list-item-text" primary={item.title} classes={{ primary: 'text-12' }} />

        {item.badge && <BpwNavBadge className="mx-4" badge={item.badge} />}
      ...
      {item.children && (
        <Collapse in={open} className="collapse-children">
          {item.children.map((_item) => (
            <BpwNavItem key={_item.id} type={`vertical-${_item.type}`} item={_item} nestedLevel={nestedLevel + 1} />
          ))}
        </Collapse>
      )}
    </ul>
...
BpwNavVerticalCollapse.propTypes = {
...
BpwNavVerticalCollapse.defaultProps = {};

const NavVerticalCollapse = withRouter(React.memo(BpwNavVerticalCollapse));
