      ...
      {variant === 'basic' ? (
        ...
        <>
          <TextField
            ...
            {...other}
          />
          <Icon className="absolute top-0 ltr:right-0 rtl:left-0 h-40 md:h-48 w-48 p-12 pointer-events-none" color="action">
            search
          </Icon>
        </>
      ) : (
  ...
  return (
    <MenuItem selected={isHighlighted} component="div">
      <ListItemIcon className="min-w-40">
        {suggestion.icon ? <Icon>{suggestion.icon}</Icon> : <span className="text-20 w-24 font-bold uppercase text-center">{suggestion.title[0]}</span>}
      </ListItemIcon>
      <ListItemText
...
function BpwSearch(props) {
  ...
  useEffect(() => {
    function itemAuthAllowed(item) {
      return BpwUtils.hasPermission(item.auth, userRole);
    }

    function setNavigation() {
      dispatch({
        type: 'setNavigation',
        value: BpwUtils.getFlatNavigation(navigation).filter((item) => itemAuthAllowed(item)),
      });
    }
            ...
            renderSuggestionsContainer={(options) => (
              ...
              >
                <div ref={suggestionsNode}>
                  <Paper elevation={1} square {...options.containerProps} style={{ width: popperNode.current ? popperNode.current.clientWidth : null }}>
                    {options.children}
...
export default withRouter(React.memo(BpwSearch));
