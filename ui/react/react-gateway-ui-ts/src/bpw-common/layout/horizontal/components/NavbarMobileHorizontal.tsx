...
function NavbarMobileHorizontal(props) {
  const classes = useStyles(props);
  const theme = useTheme();

  return (
    <div className="flex flex-col h-full overflow-hidden">
      <AppBar color="primary" position="static" elevation={0} className="flex flex-row items-center flex-shrink h-48 md:h-64 min-h-48 md:min-h-64 px-12">
      ...
      <BpwScrollbars className={clsx(classes.content)}>
        <UserNavbarHeader />
...
export default React.memo(NavbarMobileHorizontal);
