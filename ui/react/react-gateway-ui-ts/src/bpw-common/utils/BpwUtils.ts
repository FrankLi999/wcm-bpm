...
class BpwUtils {
  ...
  static setRoutes(config, defaultAuth = null) {
  ...
  static generateRoutesFromConfigs(configs, defaultAuth = null) {
    let allRoutes = [];
  ...
  static matchRoutes(routes, pathname, /*not public API*/ branch = []) {
    routes.some(route => {
      ...
      if (match) {
        ...
        if (route.routes) {
          BpwUtils.matchRoutes(route.routes, pathname, branch);
        }
      }
