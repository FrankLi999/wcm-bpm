...
import RateLimiterSelectorDialog from './SelectorDialog';
...
const rows = [
  createData('wcm-rateLimiter', true),
  createData('wcm-rateLimiter1', true),
  createData('bpm-rateLimiter2', true),
  createData('bpm-rateLimiter3', true),
  createData('bpm-rateLimiter4', true),
];

function RateLimiterSelectorRow(props) {
...
function RateLimiterSelectors(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSelectorDialog({ selectorType: 'rateLimiter' }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSelectorDialog({ selectorType: 'rateLimiter', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', margin: '2.4rem' }}>
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <RateLimiterSelectorRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
      ...
      <RateLimiterSelectorDialog />
    </div>
  );
}

export default RateLimiterSelectors;
