...
import FirewallSelectorDialog from './SelectorDialog';
...
const rows = [
  createData('wcm-firewall', true),
  createData('wcm-firewall1', true),
  createData('bpm-firewall2', true),
  createData('bpm-firewall3', true),
  createData('bpm-firewall4', true),
];

function FirewallSelectorRow(props) {
...
function FirewallSelectors(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSelectorDialog({ selectorType: 'firewall' }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSelectorDialog({ selectorType: 'firewall', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', margin: '2.4rem' }}>
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <FirewallSelectorRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
      ...
      <FirewallSelectorDialog />
    </div>
  );
}

export default FirewallSelectors;
