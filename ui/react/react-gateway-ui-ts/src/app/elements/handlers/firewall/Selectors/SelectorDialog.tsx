...
function FirewallSelectorDialog(props) {
  ...
  useEffect(() => {
     ...
     * After Dialog Open
     */
    if (selectorDialog.props.open && selectorDialog.selectorType === 'firewall' && selectorDialog.isSelector) {
      // initDialog();
      setOpen(true);
    } else {
      setOpen(false);
  ...
  function closeSelectorDialog() {
    return selectorDialog.type === 'edit'
      ? dispatch(closeEditSelectorDialog({ selectorType: 'firewall' }))
      : dispatch(closeNewSelectorDialog({ selectorType: 'firewall' }));
  }
...
export default FirewallSelectorDialog;
