...
import { closeNewSelectorRuleDialog, closeEditSelectorRuleDialog } from '../../../../store/handlers/handlerDialogSlice';

function SpringCloudSelectorRuleDialog(props) {
  const { dialogType, data } = props;
  const dispatch = useDispatch();
  const selectorRuleDialog = useSelector(({ handlers }) => handlers.dialog.handlerDialog);
  ...
  useEffect(() => {
     ...
     * After Dialog Open
     */
    if (selectorRuleDialog.props.open && selectorRuleDialog.selectorType === 'springCloud' && !selectorRuleDialog.isSelector) {
      // initDialog();
      setOpen(true);
    } else {
      setOpen(false);
    }
  }, [selectorRuleDialog.props.open]);

  function closeSelectorRuleDialog() {
    return selectorRuleDialog.type === 'edit'
      ? dispatch(closeEditSelectorRuleDialog({ selectorType: 'springCloud' }))
      : dispatch(closeNewSelectorRuleDialog({ selectorType: 'springCloud' }));
  }
    ...
    >
      <DialogTitle id="form-dialog-title" elevation={1}>
        <Typography variant="subtitle1" color="inherit">
          {selectorRuleDialog.type === 'new' ? 'New Selector Rule' : 'Edit Selector Rule'}
        </Typography>
      </DialogTitle>
      <Formik
        initialValues={{
          ...selectorRuleDialog.data,
        }}
...
export default SpringCloudSelectorRuleDialog;
