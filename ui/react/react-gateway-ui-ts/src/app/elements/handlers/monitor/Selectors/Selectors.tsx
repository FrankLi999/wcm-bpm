...
import MonitorSelectorDialog from './SelectorDialog';
...
const rows = [
  createData('wcm-Monitor', true),
  createData('wcm-Monitor1', true),
  createData('bpm-Monitor2', true),
  createData('bpm-Monitor3', true),
  createData('bpm-Monitor4', true),
];

function MonitorSelectorRow(props) {
...
function MonitorSelectors(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSelectorDialog({ selectorType: 'monitor' }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSelectorDialog({ selectorType: 'monitor', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <MonitorSelectorRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
