...
function MonitorSelectorDialog(props) {
  ...
  useEffect(() => {
     ...
     * After Dialog Open
     */
    if (selectorDialog.props.open && selectorDialog.selectorType === 'monitor' && selectorDialog.isSelector) {
      // initDialog();
      setOpen(true);
    } else {
      setOpen(false);
  ...
  function closeSelectorDialog() {
    return selectorDialog.type === 'edit'
      ? dispatch(closeEditSelectorDialog({ selectorType: 'monitor' }))
      : dispatch(closeNewSelectorDialog({ selectorType: 'monitor' }));
  }
...
export default MonitorSelectorDialog;
