...
import DivideSelectorDialog from './SelectorDialog';
...
const rows = [
  createData('wcm-Divide', true),
  createData('wcm-Divide1', true),
  createData('bpm-Divide2', true),
  createData('bpm-Divide3', true),
  createData('bpm-Divide4', true),
];

function DivideSelectorRow(props) {
...
function DivideSelectors(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSelectorDialog({ selectorType: 'divide' }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSelectorDialog({ selectorType: 'divide', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', margin: '2.4rem' }}>
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <DivideSelectorRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
      ...
      <DivideSelectorDialog />
    </div>
  );
}

export default DivideSelectors;
