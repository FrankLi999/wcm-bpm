...
function DivideSelectorDialog(props) {
  ...
  useEffect(() => {
     ...
     * After Dialog Open
     */
    if (selectorDialog.props.open && selectorDialog.selectorType === 'divide' && selectorDialog.isSelector) {
      // initDialog();
      setOpen(true);
    } else {
      setOpen(false);
  ...
  function closeSelectorDialog() {
    return selectorDialog.type === 'edit'
      ? dispatch(closeEditSelectorDialog({ selectorType: 'divide' }))
      : dispatch(closeNewSelectorDialog({ selectorType: 'divide' }));
  }
...
export default DivideSelectorDialog;
