...
import HystrixSelectorDialog from './SelectorDialog';
...
const rows = [
  createData('wcm-Hystrix', true),
  createData('wcm-Hystrix1', true),
  createData('bpm-Hystrix2', true),
  createData('bpm-Hystrix3', true),
  createData('bpm-Hystrix4', true),
];

function HystrixSelectorRow(props) {
...
function HystrixSelectors(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSelectorDialog({ selectorType: 'hystrix' }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSelectorDialog({ selectorType: 'hystrix', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', margin: '2.4rem' }}>
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <HystrixSelectorRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
      ...
      <HystrixSelectorDialog />
    </div>
  );
}

export default HystrixSelectors;
