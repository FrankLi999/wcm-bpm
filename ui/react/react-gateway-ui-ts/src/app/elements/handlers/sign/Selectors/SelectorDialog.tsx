...
function SignSelectorDialog(props) {
  ...
  useEffect(() => {
     ...
     * After Dialog Open
     */
    if (selectorDialog.props.open && selectorDialog.selectorType === 'sign' && selectorDialog.isSelector) {
      // initDialog();
      setOpen(true);
    } else {
      setOpen(false);
  ...
  function closeSelectorDialog() {
    return selectorDialog.type === 'edit'
      ? dispatch(closeEditSelectorDialog({ selectorType: 'sign' }))
      : dispatch(closeNewSelectorDialog({ selectorType: 'sign' }));
  }
...
export default SignSelectorDialog;
