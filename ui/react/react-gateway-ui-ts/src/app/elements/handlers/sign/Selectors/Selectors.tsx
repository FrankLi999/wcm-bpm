...
import SignSelectorDialog from './SelectorDialog';
...
const rows = [
  createData('wcm-Sign', true),
  createData('wcm-Sign1', true),
  createData('bpm-Sign2', true),
  createData('bpm-Sign3', true),
  createData('bpm-Sign4', true),
];

function SignSelectorRow(props) {
...
function SignSelectors(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSelectorDialog({ selectorType: 'sign' }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSelectorDialog({ selectorType: 'sign', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <SignSelectorRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
