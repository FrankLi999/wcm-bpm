...
import RewriteSelectorDialog from './SelectorDialog';
...
const rows = [
  createData('wcm-Rewrite', true),
  createData('wcm-Rewrite1', true),
  createData('bpm-Rewrite2', true),
  createData('bpm-Rewrite3', true),
  createData('bpm-Rewrite4', true),
];

function RewriteSelectorRow(props) {
...
function RewriteSelectors(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSelectorDialog({ selectorType: 'rewrite' }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSelectorDialog({ selectorType: 'rewrite', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', margin: '2.4rem' }}>
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <RewriteSelectorRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
      ...
      <RewriteSelectorDialog />
    </div>
  );
}

export default RewriteSelectors;
