...
import RewriteSelectorRuleDialog from './SelectorRuleDialog';
...
function RewriteSelectorRuleRow(props) {
...
function RewriteSelectorRules(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSelectorRuleDialog({ selectorType: 'rewrite' }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSelectorRuleDialog({ selectorType: 'rewrite', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', margin: '2.4rem' }}>
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <RewriteSelectorRuleRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
      ...
      <RewriteSelectorRuleDialog />
    </div>
  );
}

export default RewriteSelectorRules;
