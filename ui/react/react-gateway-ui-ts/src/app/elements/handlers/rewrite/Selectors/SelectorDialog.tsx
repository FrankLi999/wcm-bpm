...
function RewriteSelectorDialog(props) {
  ...
  useEffect(() => {
     ...
     * After Dialog Open
     */
    if (selectorDialog.props.open && selectorDialog.selectorType === 'rewrite' && selectorDialog.isSelector) {
      // initDialog();
      setOpen(true);
    } else {
      setOpen(false);
  ...
  function closeSelectorDialog() {
    return selectorDialog.type === 'edit'
      ? dispatch(closeEditSelectorDialog({ selectorType: 'rewrite' }))
      : dispatch(closeNewSelectorDialog({ selectorType: 'rewrite' }));
  }
...
export default RewriteSelectorDialog;
