...
function RequestHandlerDialog(props) {
  ...
  useEffect(() => {
     ...
     * After Dialog Open
     */
    if (systemDialog.props.open && systemDialog.dialogType === 'handler') {
      // initDialog();
      setOpen(true);
    } else {
      setOpen(false);
  ...
  function closeSystemDialog() {
    return systemDialog.type === 'edit'
      ? dispatch(closeEditSystemDialog({ dialogType: 'handler' }))
      : dispatch(closeNewSystemDialog({ dialogType: 'handler' }));
  }
    ...
    >
      <DialogTitle id="form-dialog-title" elevation={1}>
        <Typography variant="subtitle1" color="inherit">
          {systemDialog.type === 'new' ? 'New Handler' : 'Edit Handler'}
        </Typography>
      </DialogTitle>
      <Formik
        initialValues={{
...
export default RequestHandlerDialog;
