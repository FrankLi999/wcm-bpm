...
function MetadataDialog(props) {
  ...
  useEffect(() => {
     ...
     * After Dialog Open
     */
    if (systemDialog.props.open && systemDialog.dialogType === 'metadata') {
      // initDialog();
      setOpen(true);
    } else {
      setOpen(false);
  ...
  function closeSystemDialog() {
    return systemDialog.type === 'edit'
      ? dispatch(closeEditSystemDialog({ dialogType: 'metadata' }))
      : dispatch(closeNewSystemDialog({ dialogType: 'metadata' }));
  }
    ...
    >
      <DialogTitle id="form-dialog-title" elevation={1}>
        <Typography variant="subtitle1" color="inherit">
          {systemDialog.type === 'new' ? 'New Metadata' : 'Edit Metadata'}
        </Typography>
      </DialogTitle>
      <Formik
        initialValues={{
