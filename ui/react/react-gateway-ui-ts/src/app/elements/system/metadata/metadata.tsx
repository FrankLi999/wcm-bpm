...
function Metadata(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSystemDialog({ dialogType: 'metadata', data: {} }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSystemDialog({ dialogType: 'metadata', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <MetadataRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
