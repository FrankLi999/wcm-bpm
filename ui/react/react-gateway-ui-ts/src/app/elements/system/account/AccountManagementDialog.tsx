...
function AccountManagementDialog(props) {
  ...
  useEffect(() => {
     ...
     * After Dialog Open
     */
    if (systemDialog.props.open && systemDialog.dialogType === 'account') {
      // initDialog();
      setOpen(true);
    } else {
      setOpen(false);
  ...
  function closeSystemDialog() {
    return systemDialog.type === 'edit'
      ? dispatch(closeEditSystemDialog({ dialogType: 'account' }))
      : dispatch(closeNewSystemDialog({ dialogType: 'account' }));
  }
    ...
    >
      <DialogTitle id="form-dialog-title" elevation={1}>
        <Typography variant="subtitle1" color="inherit">
          {systemDialog.type === 'new' ? 'New Account' : 'Edit Account'}
        </Typography>
      </DialogTitle>
      <Formik
        initialValues={{
