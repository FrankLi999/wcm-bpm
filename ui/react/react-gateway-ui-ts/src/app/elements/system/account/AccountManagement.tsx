...
function AccountManagementRow(props) {
...
function AccountManagement(props) {
  ...
  const handleNewDialog = () => {
    dispatch(openNewSystemDialog({ dialogType: 'account', data: {} }));
  };

  const handleEditDialog = (row, rowIndex) => {
    dispatch(openEditSystemDialog({ dialogType: 'account', rowIndex: rowIndex, data: { title: rowIndex, notes: rowIndex } }));
  };
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {(rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, rowIndex) => (
                <AccountManagementRow key={row.name} row={row} rowIndex={rowIndex} onEdit={handleEditDialog} onRemoveRow={removeRow} />
              ))}
            </TableBody>
          </Table>
          <TableFooter>
