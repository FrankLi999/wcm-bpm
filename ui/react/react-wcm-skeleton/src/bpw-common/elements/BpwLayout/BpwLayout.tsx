...
import BpwLayouts from '../../layout/BpwLayouts';
...
function BpwLayout(props) {
  const dispatch = useDispatch();
  const settings = useSelector(({ bpw }) => bpw.settings.current);
  const defaultSettings = useSelector(({ bpw }) => bpw.settings.defaults);
  ...
  const Layout = useMemo(() => BpwLayouts[settings.layout.title], [settings.layout.title]);

  return _.isEqual(newSettings.current, settings) ? <Layout classes={{ root: classes.root }} {...props} /> : null;
}

export default React.memo(BpwLayout);
