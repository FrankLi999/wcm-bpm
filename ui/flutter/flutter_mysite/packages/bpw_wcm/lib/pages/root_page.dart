...
class _RootPageState extends State<RootPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<SiteConfigBloc, SiteConfigState>(
      ...
      if (state is PublicSiteConfigUpdated) {
        currentWidget = _createNestedNavigator(state);
      } else if (state is ProtectedSiteConfigUpdated) {
        currentWidget = _createNestedNavigator(state);
      } else {
        currentWidget = AppContext.loadingScreen;
      }
  ...
  Widget _createNestedNavigator(SiteConfigState state) {
    return ("drawer" == _navigatorPosition())
        ? NestedNavigators(
