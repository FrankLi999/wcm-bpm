...
public abstract class HalCachingLinkResolver implements HalLinkResolver {
	...
	public List<HalResource<?>> resolveLinks(String[] linkedIds, ProcessEngine processEngine) {
		...
		if (cache == null) {
			...
			if (!notCachedLinkedIds.isEmpty()) {
				List<HalResource<?>> notCachedResources = resolveNotCachedLinks(
						notCachedLinkedIds.toArray(new String[notCachedLinkedIds.size()]), processEngine);
	 ...
	 */
	protected List<HalResource<?>> resolveCachedLinks(String[] linkedIds, Cache cache,
	...
	/**
	 * @return a comparator for this HAL resource if not overridden sorting is
	 *         skipped
	 */
