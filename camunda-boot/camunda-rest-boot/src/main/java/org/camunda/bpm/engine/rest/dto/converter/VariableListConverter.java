...
/**
 * Reads a list of {@link VariableQueryParameterDto}s from a single parameter.
 * Expects a given format (see method comments).
 * 
...
public class VariableListConverter extends JacksonAwareStringToTypeConverter<List<VariableQueryParameterDto>> {
	...
	/**
	 * Expects a query parameter of multiple variable expressions formatted as
	 * KEY_OPERATOR_VALUE, e.g. aVariable_eq_aValue. Multiple values are expected to
	 * be comma-separated.
	 */
	@Override
