...
package com.bpwizard.gateway.admin.service.init;
...
public class LocalDataSourceLoader implements InstantiationAwareBeanPostProcessor {
    ...
    protected void init(final DataSourceProperties properties) {
        // If jdbcUrl in the configuration file specifies the gateway database, it is removed,
        // because the gateway database does not need to be specified when executing the SQL file,
        // otherwise the gateway database will be disconnected when the gateway database does not exist
        String jdbcUrl = StringUtils.replace(properties.getUrl(), "/gateway?", "?");
    ...
    private void execute(final Connection conn) throws Exception {
        ...
        LOGGER.info("execute gateway schema sql: {}", SCHEMA_SQL_FILE);
